...
**` %>% ` ** pipes the output of one function to the input of another function. Great for improving readability of your code!
--
...
```{r, echo=TRUE, eval=FALSE}
Orange <- Orange %>% arrange(circumference) 
```
...
```{r, echo=TRUE, eval=TRUE}
Orange %>% arrange(desc(circumference)) %>% head()
```
...
```
.footnote[Add a minus sign before any column name to remove it!]
---
...
```
--
Use `transmute()` to get rid of the other columns:
--
```{r, echo=TRUE, eval=TRUE}
...
```
--
You can also select only one of the columns:
...
- Create a new column which is the ratio of circumference to age and name it as `growth`.
...
answer <- Orange %>% mutate(growth=circumference/age) %>% 
  filter(Tree==3, growth==min(growth))
print(answer$age)
```
...
- `n()` calculates the number of rows in the group.
--
...
```
--
Click [here](https://dplyr.tidyverse.org/articles/two-table.html) to learn more about the two table verbs.
---
...
- [Data wrangling, exploration, and analysis with R](http://stat545.com/block009_dplyr-intro.html) lecture notes by Jenny Bryan


- [Biomedical Data Science](http://genomicsclass.github.io/book/pages/dplyr_tutorial.html) lecture notes by Rafael Irizarry and Michael Love
]